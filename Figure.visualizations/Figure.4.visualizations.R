...
shannon <- read_excel("Supplementary table 17.xlsx")

shannon$Normalized <- shannon$Shannon / max(shannon$Shannon)
shannon$mean <- case_when(
  shannon$Diagnosis == "HC" ~ mean((shannon %>% filter(Diagnosis == "HC"))$Normalized),
  shannon$Diagnosis == "DCE" ~ mean((shannon %>% filter(Diagnosis == "DCE"))$Normalized),
  shannon$Diagnosis == "DRE" ~ mean((shannon %>% filter(Diagnosis == "DRE"))$Normalized))
shannon$Diagnosis <- factor(shannon$Diagnosis, levels = c("HC", "DCE", "DRE"))
...
tcrdb <- read.csv("~/TCR-db.csv", fill = T)
